...
This example demonstrates all production best practices combined in a single configuration: remote state, modules, tagging, encryption, and CI/CD readiness.

## Files

| File | Description |
|------|-------------|
| `main.tf` | Complete setup: remote backend, provider, VPC module, EC2, RDS, S3, CloudWatch |
| `variables.tf` | Well-documented variables with validation, types, and descriptions |
...
| **Encryption** | S3 SSE, RDS encryption, KMS |
| **Tagging** | Consistent tags across all resources |
| **Modularity** | Reusable VPC module with versioning |
| **IAM** | Least privilege roles and policies |
| **Monitoring** | CloudWatch alarms and dashboards |
| **Security** | Sensitive variables, secure defaults |
| **Backup** | Automated RDS snapshots |

## Try It

```bash
# 1. First deploy the backend infrastructure
cd backend
terraform init && terraform apply

# 2. Then deploy the main infrastructure
cd ../infra
terraform init && terraform apply
```
